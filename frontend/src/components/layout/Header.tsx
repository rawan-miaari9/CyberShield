    ...
    <header className="h-[76px] px-7 bg-[#0c1220]/95 backdrop-blur border-b border-slate-800/80 flex items-center justify-between sticky top-0 z-20 shrink-0">
      ...
      <div className="flex items-center gap-3.5">
        ...
        <div className="relative" ref={bellRef}>
          ...
          {bellOpen && (
            <div className="absolute right-0 top-full mt-2 w-[400px] max-w-[calc(100vw-2rem)] rounded-2xl border border-slate-800 bg-[#0e1626] shadow-2xl shadow-black/50 overflow-hidden z-50">
              ...
              <div className="max-h-[320px] overflow-y-auto divide-y divide-slate-800/70">
                {latest.map((n) => (
                  <div key={n.id} className="px-5 py-3.5 flex items-start gap-3 hover:bg-white/[0.02]">
                    <span className={`mt-1.5 w-2 h-2 rounded-full shrink-0 ${n.read ? 'bg-slate-700' : 'bg-cyan-400'}`} />
                    <button
                      type="button"
                      onClick={() => {
                        setBellOpen(false);
                        // Viewing a notification marks it read, so the bell
                        // badge decreases immediately (failure stays silent
                        // here — the row keeps its unread dot and the
                        // dedicated Mark read action still reports errors).
                    ...
                    >
                      <p className="text-[15px] font-medium text-slate-100 truncate">{n.title}</p>
                      <p className="text-[14.5px] text-slate-400 truncate mt-0.5">{n.message}</p>
                    </button>
                    {!n.read && (
                      <button
                        type="button"
                        ...
                        className="text-xs text-slate-500 hover:text-cyan-300 shrink-0 mt-0.5"
                      >
                        Mark read
                      </button>
                    )}
              ...
              <Link
                ...
                className="block px-4 py-3 text-center text-sm font-medium text-cyan-300 hover:text-cyan-200 hover:bg-white/[0.02] border-t border-slate-800/80"
              >
